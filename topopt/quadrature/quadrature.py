...
def kappa(x1, x2, y1, y2):
    if squared_norm(x1, x2, y1, y2) <= 8:
        return 1.0
    elif squared_norm(x1, x2, y1, y2) <= 25.0 / 2:
        return f((squared_norm(x1, x2, y1, y2) - 8) / (9.0 / 2))
    else:
        return 0.0


def int_jk(x1, x2, y1, y2, sigma):
    return np.power(squared_norm(x1, x2, y1, y2), -1 - 1 * sigma) * kappa(
        x1, x2, y1, y2
    )
...
def int_21(x, y, sigma):
    return (
        tilde_h2(1, x, y, sigma) + tilde_h2(x, 1, y, sigma) + tilde_h2(x, y, 1, sigma)
    )
...
def save_weights(sigma):
    ...
    for k in range(len(indexes)):
        ...
        ints["int" + str(4 + k)] = integrate.nquad(
            int_jk,
            [
                [0, 1],
                [0, 1],
